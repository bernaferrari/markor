...
# Compose endpoint. Markor keeps filled and outlined vectors for each icon in
# one <IconName>Icon.kt file and exposes them through MaterialSymbols.kt.

set -euo pipefail

readonly ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
readonly OUTPUT_DIR="$ROOT_DIR/shared/src/commonMain/kotlin/com/bernaferrari/remarkor/ui/icons"
readonly BASE_URL="https://fonts.gstatic.com/render/v1/Material+Symbols+Rounded/24dp"
readonly PACKAGE_NAME="com.bernaferrari.remarkor.ui.icons"

readonly FILLED_ICONS=(
  add archive arrow_back arrow_downward arrow_forward arrow_upward article assignment
  brightness_auto center_focus_strong center_focus_weak check check_box check_circle
  chevron_right clear close cloud_off code content_copy content_cut create dark_mode
  delete delete_forever delete_outline description drag_indicator edit edit_note
  error_outline expand_less expand_more file_download find_replace folder folder_open
  folder_zip format_bold format_italic format_list_bulleted format_list_numbered
  format_quote format_strikethrough grid_view history image image_not_supported info
  keyboard_arrow_up label light_mode link list lock more_vert notes palette push_pin
  redo remove restore search search_off select_all settings share sort star text_snippet
  title toc undo visibility
)

readonly OUTLINED_ICONS=(
  add archive check_circle delete description folder list lock settings star
)

readonly AUTO_MIRRORED_FILLED_ICONS=(
  arrow_back arrow_forward article assignment format_list_bulleted label list notes redo
  sort text_snippet toc undo
)
...
download_vector() {
  ...
  curl \
    ...
    --connect-timeout 15 \
    --max-time 60 \
    --user-agent 'Markor Material Symbols generator' \
    "$url" \
    --output "$output"
}
