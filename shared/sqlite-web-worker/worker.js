...
function openRequest(id, requestData) {
    try {
        ...
        const fileName = requestData.fileName || 'markor_notes.db';
        // Room 3 / SQLite WASM: prefer OPFS (needs COOP+COEP). Fall back to oo1.DB
        // when OpfsDb is unavailable (insecure context / missing isolation).
        // See https://sqlite.org/wasm/doc/trunk/api-oo1.md
        let newDatabase;
        if (typeof sqlite3.oo1.OpfsDb === 'function') {
            try {
                newDatabase = new sqlite3.oo1.OpfsDb(fileName);
            } catch (opfsErr) {
                console.warn('OpfsDb open failed, falling back to oo1.DB:', opfsErr);
                newDatabase = new sqlite3.oo1.DB(fileName, 'ct');
            }
        } else if (sqlite3.oo1 && typeof sqlite3.oo1.DB === 'function') {
            console.warn('sqlite3.oo1.OpfsDb unavailable; using oo1.DB (enable COOP/COEP for OPFS)');
            newDatabase = new sqlite3.oo1.DB(fileName, 'ct');
        } else {
            throw new Error('No usable sqlite3.oo1 database constructor (OpfsDb/DB)');
        }
